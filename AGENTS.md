...
このファイルは、AIコーディングエージェント（Claude Code・GitHub Copilot・OpenAI Codex・Cursor など）がこのリポジトリで作業する際のガイダンスを提供します（[agents.md](https://agents.md/) 規約）。Claude Code 固有の設定は `CLAUDE.md` に置き、`CLAUDE.md` は本ファイルを `@AGENTS.md` でインポートする。

## このファイルの編集方針

**AGENTS.md は（Claude Code では `CLAUDE.md` からのインポート経由で）毎リクエストの先頭に全文が読み込まれる。** 記述を増やすことは常時のコンテキスト消費と、本当に守らせたいルールの埋没を同時に招く。追記の前に必ず以下を確認すること。

- **載せるのは「知らないと壊すこと」だけ。** コマンド・運用ルール・コードスタイル、そして**不変条件**（「X は必ず Y を経由する」「Z を使ってよいのはここだけ」）に限る。ディレクトリ構成やレイヤーの責務は `README.md` / `docs/` の担当。
- **経緯の置き場所を使い分ける。** 設計判断の結論と理由は `docs/` に、時系列の経緯・調査ログ・一時的な運用手順・改訂履歴は git / PR / Issue 履歴に置く（#185）。設計書（`docs/`）に後者を残さない。
...
- **目安として `AGENTS.md` と `CLAUDE.md` の合計で 200行 / 12,000文字を超えない。** 超えたら追記ではなく `docs/` への移動を行う。
- **ツール固有の設定は `CLAUDE.md` 等の各ツールのファイルに置く。** 本ファイルには、どのエージェントにも共通するルールだけを書く。
- 適用済みマイグレーション SQL（`supabase/migrations/`）のコメントは旧名の `CLAUDE.md` を参照したまま残っている（適用済みファイルは履歴として書き換えない）。本ファイルの該当節を指していると読み替える。
