...
--
-- タイムスタンプについて: 演習seed（20260524000000_seed_gas_advanced_exercises.sql）
-- よりフレッシュ環境での適用順を前にするため意図的に過去日付を選んでおり、
-- 実際の適用日時ではない。そのためリモートには既に20260614080707/20260715233228が
-- 適用済みバージョンとして記録されており、本ファイルより後のバージョンが先に
-- 存在する状態になる。db push はそのままでは拒否するため、内容が本番の実値と
-- 一致することをSELECTで確認済みの前提で
-- `supabase migration repair --status applied 20260521000000` を実行して
-- 適用済みとして記録する（docs/database.md 7.1節参照）。
-- =====================================================
