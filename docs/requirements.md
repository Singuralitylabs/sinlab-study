...
| ステータス | 説明 |
|:--|:--|
| `trial` | 承認待ち = **お試し（trial）ユーザー**。ログインしてサービスを利用できるが、閲覧・提出できるのはお試し公開コンテンツ（`is_open_to_trial = true`）のみ。承認待ちであることはアプリ内バナーで通知する |
| `active` | 承認済み（学習サービスに制限なくアクセス可能） |
| `rejected` | 却下済み（`/rejected` 却下画面へリダイレクト） |

お試し体験を通じた入会動機の醸成のため、承認前でも通常どおりログインでき、お試し公開指定されたコンテンツのみ閲覧・課題提出できる（詳細は「3.2 学習コンテンツ配信」および[機能設計書](./specification.md)を参照）。
...
上表はロール（`role`）による権限を示す。これとは別に、ステータス（`status`）による制限が重なる: お試しユーザー（`trial`）は「コンテンツ閲覧」「課題提出」「自分の進捗確認」の対象がお試し公開コンテンツのみに制限される。`active` は上表のとおり制限を受けない。
...
- 初回ログイン時にユーザーを自動登録（`status=trial`）し、そのままダッシュボードへ遷移。承認前でもお試しユーザーとしてサービスを利用できる
- 認証済みかつ `active` または `trial`（お試し）ステータスのユーザーがコンテンツにアクセス可能。`rejected` は遮断する
- お試しユーザーには承認待ちであることをアプリ内バナーで通知（承認待ち専用画面 `/pending` は設けない）
...
- `is_open_to_trial` フラグでお試しユーザーへの公開 / 非公開を制御（コンテンツ管理画面のチェックボックス「お試しユーザーにも公開する」で設定）
- `is_deleted` フラグで論理削除（データ保全）
- `display_order` で表示順を制御

#### お試しユーザーへの表示制御
- コースツリー（テーマ / フェーズ / 週 / コンテンツ一覧）は全件表示し、お試し非公開のコンテンツは鍵アイコンでロックする（何が学べるかを見せて入会動機につなげるため）
- ロック済みコンテンツは中身を表示しない。直リンクでアクセスした場合もロック画面を表示する
- ダッシュボードの進捗率は、お試しユーザーではお試し公開コンテンツのみを分母とする（体験範囲内の進捗を示す）
- 承認前の提出・進捗は承認後もそのまま引き継がれる
...
- Supabase Authによる認証
- プロキシ（`proxy.ts`）によるページ単位のアクセス制御
- 承認済み（active）ユーザーは全公開コンテンツ、お試し（trial）ユーザーはお試し公開コンテンツのみアクセス可。却下済み（rejected）は遮断
...
| Slack Incoming Webhooks | 初回ログイン時の承認依頼通知 | 実装済み |
...
| 2026年7月 | 承認前ユーザーを「お試し（trial）ユーザー」と定義し、お試し公開コンテンツの閲覧・課題提出を許可する要件を追加。`/pending` 承認待ち画面の廃止、`is_open_to_trial` フラグ、ロック表示、ユーザー種別・権限・セキュリティの記述を更新 |
| 2026年9月 | #88対応：`users.status` の値 `'pending'` を `'trial'` にリネーム。2.2節・2.3節・3.1節・5.3節のステータス値表記を更新し、暫定対応だった旨の注記を削除 |
