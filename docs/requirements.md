...
| ステータス | 説明 |
|:--|:--|
| `pending` | 承認待ち = **お試し（trial）ユーザー**。ログインしてサービスを利用できるが、閲覧・提出できるのはお試し公開コンテンツ（`is_open_to_trial = true`）のみ。承認待ちであることはアプリ内バナーで通知する |
| `active` | 承認済み（学習サービスに制限なくアクセス可能） |
| `rejected` | 却下済み（`/rejected` 却下画面へリダイレクト） |

お試し体験を通じた入会動機の醸成のため、承認前でも通常どおりログインでき、お試し公開指定されたコンテンツのみ閲覧・課題提出できる（詳細は「3.2 学習コンテンツ配信」および[機能設計書](./specification.md)を参照）。DB上の値 `'pending'` 自体のリネームは別issueで対応予定のため、新設するフラグ名・UI文言にのみ trial 系の名称を用いる。
...
上表はロール（`role`）による権限を示す。これとは別に、ステータス（`status`）による制限が重なる: お試しユーザー（`pending`）は「コンテンツ閲覧」「課題提出」「自分の進捗確認」の対象がお試し公開コンテンツのみに制限される。`active` は上表のとおり制限を受けない。
...
- 初回ログイン時にユーザーを自動登録（`status=pending`）し、そのままダッシュボードへ遷移。承認前でもお試しユーザーとしてサービスを利用できる
- 認証済みかつ `active` または `pending`（お試し）ステータスのユーザーがコンテンツにアクセス可能。`rejected` は遮断する
