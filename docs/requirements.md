...
| [認証設計書](./auth-design.md) | 独立認証（Googleログイン + ユーザー承認フロー） |
| [認証実装ワークフロー](./workflow-auth-implementation.md) | 認証機能の実装手順 |
...
- 独立認証基盤: 本アプリ独自のGoogle OAuth + ユーザー承認フロー（詳細は[認証設計書](./auth-design.md)を参照）
...
| シンラボポータルサイト | 参考: 同一Googleアカウントでの利用 | 連携なし（独立運用） |
