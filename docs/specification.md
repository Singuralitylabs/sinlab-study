...
5. アップロード結果の検証（`upload()` の戻り値 `data.path` が存在し、保存先キーと一致すること）
6. 実体の存在確認（`list(<folder>, { search: <ファイル名> })` で対象オブジェクトを**名前の完全一致**で照合。`search` は完全一致ではないため、`slide-01.pdf.bak` のような前方一致の別ファイルを存在扱いにしない）
   - 5・6 のいずれかを満たせない場合は500を返し、**URLを組み立てない**（実体の無い `pdf_url` がコンテンツに保存されるのを防ぐ）
   - 確認に失敗してもアップロード済みオブジェクトは削除しない（一時的な `list()` 失敗で正常なファイルを消さないため）。自動採番経路でこの状態になった場合は、番号を明示指定して再アップロードする
