...
- Stripeサブスク契約中（現在契約中とみなせるステータスの `stripe_subscriptions` 行を持つ）ユーザーは、`membership_type` と課金状態の不整合を防ぐため会員種別のセレクトボックスを無効化し、「Stripe側で解約後に変更してください」の注記を表示する。契約状況を取得できない場合も同様に無効化する（フェイルクローズ。`PATCH /api/admin/users` の `change_membership` も同じ判定をAPI側で行う）
...
- `change_membership` では `membershipType` が必須（未指定・不正値は 400）。対象が `active` 以外・存在しない・削除済みのいずれかで0行更新の場合は 409。対象ユーザーがStripeサブスク契約中の場合、または契約状況を取得できない場合も 409 / 503（フェイルクローズ。2.7節参照）
...
| 409 | `approve` の重複承認、`change_membership` の対象不正・Stripe契約中 |
| 500 | DB 更新失敗 / サーバーエラー |
...
| 2026年9月 | 承認済み（active）ユーザーの会員種別変更機能を追加（#95）：2.7節・6.1.3節を更新。Stripe契約中ユーザーは変更UI・APIともに無効化 |
