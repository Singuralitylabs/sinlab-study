...
- Checkout手続き中に管理者が手動承認した場合: 決済完了時点で一般有料会員として上書きされる（許容）。降格側は `membership_type=general` ガードで巻き込みを防止するため、逆方向（決済完了後に管理者が誤って却下）の影響は限定的
