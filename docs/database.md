...
5. 「GAS学習（応用編）」「GAS学習（実践編）」ともにテーマ作成SQLの欠落は解消済み（上記「判明した事実」4参照）。
