...
カラム削除・リネーム・型変更など既存データに影響する破壊的変更を含む場合は、本番反映前に Wiki の [本番環境リリース手順](https://github.com/Singuralitylabs/sinlab-study/wiki/本番環境リリース手順)（Step 3 にDBマイグレーション、末尾にロールバック方針を記載）に従うこと。旧コードと新DB（またはその逆）の両立が取れない移行の段取りは [`docs/breaking-migration-release.md`](./breaking-migration-release.md) を参照する。
...
| 2026年9月 | #219対応：後方互換でないデータ移行のリリース手順（`docs/breaking-migration-release.md`）を新設。7.2節から参照 |
