...
スライドPDFを保存する `slides` は**非公開バケット**（`public = false`、#89）。参照はコンテンツの可視性に連動させ、書き込み系はコンテンツ管理者に限定する。
...
| `20260908000000_secure_slides_bucket.sql` | スライドPDFの署名付きURL配信（#89）: `slides` バケットを非公開化し、`learning_contents.pdf_url` を公開URLからオブジェクトキーへ一括正規化、`storage.objects` に `slides` の SELECT（`learning_contents` の RLS に委譲）/ INSERT / UPDATE / DELETE ポリシーを追加。**アプリ側の署名付きURL配信と同時にリリースすること**（旧コードは pdf_url を公開URLとして組み立てるため） |
