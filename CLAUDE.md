...
`supabase/migrations/` **直下にフラットな連番SQLファイルで管理する（サブディレクトリを作らない）**。Supabase CLIの `migration list` / `db push` はサブディレクトリを再帰走査しないため（#149）。ファイル名は `<3桁連番>_<説明>.sql` とし、連番の大きい方が後から適用される。RLSポリシーが参照するカラム・ヘルパー関数は、先により小さい連番のファイルで追加されている必要がある。区分・適用手順・リモート適用履歴との整合手順は `docs/database.md` 7章を参照。
