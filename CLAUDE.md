...
- **提出API・進捗APIの可視性チェックは通常クライアントの SELECT で行う。** `contentId` を `is_published = true` 付きで SELECT して0行なら403。RLSがステータスを織り込むため、アプリ層でステータス分岐を書かない（`is_published` の絞り込みのみ明示し、admin / maintainer のプレビュー中でも未公開コンテンツへの進捗登録・提出・AIレビューは行わせない）。
