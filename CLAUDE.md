...
- **設計書（`docs/`）に経緯・調査ログ・一時的な運用手順・改訂履歴を残さない。** 変更履歴は git / PR / Issue 履歴で管理する（#185）。
