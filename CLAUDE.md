...
**AIレビューへの講座資料の受け渡し**: `POST /api/ai-review` は演習提出をレビューする際、演習指示・模範回答（`reference_answer`）・ヒント（`hint`）に加えて、同じ週（`week_id`）の公開スライド（`content_type='slide'` かつ `is_published=true` かつ `is_deleted=false`）の `slide_markdown` を `display_order` 順に連結し、「講座資料」としてGeminiへのプロンプトに条件付きで挿入する（`app/services/api/gemini.ts` の `generateReview()` 第5引数 `courseMaterial`）。資料は単純な文字数カットではなく、`generateReview()` の呼び出しとは別に要約専用のGemini呼び出しを1回追加し、連結した講座資料を `MAX_MATERIAL_LENGTH`（2000文字）以内に要約したうえで `courseMaterial` としてレビュー生成プロンプトへ組み込む2段階構成とする（文の途中で不自然に切れることを避けるため）。この要約呼び出しの分、レビュー1件あたりAPI呼び出しが1回増え、レイテンシ・コストがわずかに増加する点は留意点として認識している。取得は提出者本人のRLSスコープのクライアントで行い、service_roleは使用しない（生徒は同じ週の公開コンテンツを閲覧できるため）。
