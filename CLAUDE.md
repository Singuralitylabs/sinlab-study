...
- 管理画面の「コンテンツ管理」から CSV をアップロードして、テーマ・フェーズ・週・コンテンツを一括登録できる
- MVP では `video` / `text` / `exercise` の3種のみ対応し、`slide` は対象外
- MVP では取り込みは「新規登録のみ」とし、既存コンテンツの更新・上書きは対象外
- CSV 1行 = 1コンテンツとして扱い、テーマ・フェーズ・週名を名前で解決し、存在しなければ新規作成する
- 取り込みは全件成功時のみ反映し、途中でエラーが発生した場合は作成済みデータをロールバックして部分反映しない
- 失敗時は行番号と原因を表示し、管理者が修正しやすいようにする
